...
const events = [
  { title: "Ganesh Sthapana", date: "14 September 2026", time: "11:00 AM", location: "K Block library", category: "pooja", icon: "🪔", description: "Join us for the ceremonial installation of Lord Ganesha." },

  { title: "Cultural Program", date: "15 September 2026", time: "11:30 AM", location: "Mandala", category: "cultural", icon: "🕺💃", description: "Put your creativity to work in our festive student competitions." },

  { title: "Visarjana & Ladoo Auction", date: "16 September 2026", time: "03:00 PM", location: "Mandala", category: "END", icon: "🕺💃", description: "" },
];
...
const categoryLabels = { pooja: "Pooja", cultural: "Cultural", auction: "END", all: "All" };
...
function eventCard(event, full = false) {
  ...
  const categoryName = categoryLabels[categoryKey] || "END";
...
function setupDonationForm() {
  ...
  form.addEventListener("submit", async event => {
    ...
    ["name", "category", "contact", "amount", "utr", "paymentDate"].forEach(key => {
      ...
      const invalid = !field || !value || (key === "amount" && Number(field.value) <= 0) || (key === "contact" && !/^\d{10}$/.test(value));
...
const expenseItems = [
  { name: "Ganesh Ideal", amount: 1500, date: "7 Sep 2026", icon: "🐘", detail: "Paid Advance" },
  { name: "Pandith", amount: 1000, date: "8 Sep 2026", icon: "👴", detail: "Paid Advance" },
  { name: "Ladoo", amount: 500, date: "8 Sep 2026", icon: "🍪", detail: "Paid Advance" },
  { name: "Invitation Cards", amount: 300, date: "9 Sep 2026", icon: "🎴", detail: "Paid Advance" },
...
function renderExpenseList() {
  const target = document.querySelector("[data-expense-list]");
  if (!target) return;

  const totalSpent = expenseItems.reduce((sum, item) => sum + item.amount, 0);
  const collectionAmount = 12741;
  const remaining = collectionAmount - totalSpent;

  const totalSpentNode = document.querySelector("[data-total-spent]");
  const totalCollectionNode = document.querySelector("[data-total-collection]");
  const remainingNode = document.querySelector("[data-remaining-balance]");

  if (totalSpentNode) totalSpentNode.textContent = formatCurrency(totalSpent);
  if (totalCollectionNode) totalCollectionNode.textContent = formatCurrency(collectionAmount);
  if (remainingNode) remainingNode.textContent = formatCurrency(remaining);

  target.innerHTML = expenseItems.map(item => `
    <article class="expense-item">
      <div class="expense-meta">
        <div class="expense-icon" aria-hidden="true">${item.icon}</div>
        <div>
          <h3 class="expense-name">${item.name}</h3>
          <p class="expense-detail">${item.detail}</p>
        </div>
      </div>
